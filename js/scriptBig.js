...
const bakingTime = 1000 * 60 * 60 * 7; // 6 hours
...
const setSlotSelected = (dataSlot) => {
    ...
    if (dataSlot.state === SlotState.IDLE) {
        ...
        //implementado
        document.getElementById(`boardSN_${dataSlot.id}`).textContent = `Board SN: ${dataSlot.boardSerialNumber}`;
        document.getElementById(`boardSN_${dataSlot.id}`).parentElement.setAttribute('data-board-sn', dataSlot.boardSerialNumber);
...
mainFormElement.addEventListener("submit", (e) => {
    ...
    //Coloca o serial entre time stamp e slot X

    const boardSNDiv = document.getElementById(`boardSN_${slotSelected.id}`);
    boardSNDiv.textContent = `${boardSerialNumber.value}`;
...
stopButton.addEventListener("click", (e) => {
    ...
    setSlotSelected(newSlot);
//implementado
    document.getElementById(`boardSN_${slotSelected.id}`).textContent = ''; // Limpar o número de série
    document.getElementById(`boardSN_${slotSelected.id}`).parentElement.removeAttribute('data-board-sn');
